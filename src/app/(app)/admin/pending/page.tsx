...
function AdminPendingBookingsContent({
  ...
  // Query construct - lock status to PENDING
    ...
    <div className="space-y-6 flex-1 flex flex-col text-[#FAFAFA]">
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-8 text-center max-w-xl mx-auto">
          ...
          <p className="mt-2 text-sm text-[#A1A1AA]">{(error as Error)?.message || 'Check connection details.'}</p>
        ...
        <div className="text-center py-20 text-[#A1A1AA] border border-dashed border-white/[0.08] rounded-xl bg-[#111113]">
          ...
          <h3 className="font-bold text-[#FAFAFA]">No Pending Reviews</h3>
          <p className="text-sm text-[#A1A1AA] mt-1">Excellent! All pending booking requests have been processed.</p>
        </div>
      ) : (
              ...
              <TableBody>
                {bookingsData.data.map((booking) => (
                  <TableRow key={booking.bookingId} className="border-b border-white/[0.04] hover:bg-[#18181B]/50 transition-colors">
                    ...
                    <TableCell>
                      <Badge className="text-[10px] px-2 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/20">
                        {booking.status}
                      </Badge>
                    </TableCell>
...
export default function AdminPendingBookingsPage({
  ...
  return (
    <Suspense fallback={<div className="py-12 text-center text-[#A1A1AA]">Initializing pending bookings table...</div>}>
